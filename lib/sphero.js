...
var COMMAND_SOP1 = 0,
    ...
    // Glob expression for Sphero serial ports
    DEFAULT_SPHERO_PORT = '/dev/tty.Sphero*';

var Sphero = function(path) {
  var self = this;

  // Path to port Sphero is on - use glob to find default port
  if(typeof path === 'undefined') {
    path = glob.sync(DEFAULT_SPHERO_PORT)[0];
    if(typeof path === 'undefined') {
      throw new Error('No Sphero found matching ' + DEFAULT_SPHERO_PORT);
    }
  }
...
Sphero.prototype.setStabilization = function(data, cb) {
  var options = {
    device: this.devices.sphero,
    command: 0x02,
    data: data
...
Sphero.prototype.setBackLED = function(val, cb) {
  var options = {
    device: this.devices.sphero,
    command: 0x21,
    data: [val],

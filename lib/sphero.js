  ...
  var processData = function(data) {
    ...
    if (self.buffer.length >= 6) {
      ...
      if (end) {
        ...
        } else if (packet[RESPONSE_MRSP] == 0x07) {
...
Sphero.prototype.disconect = function() {
  this.port.close();
};
...
Sphero.prototype.setCollisionDetection = function (data, _onHit) {
  this.onHit = _onHit || function() {return;};
  data = data || [0x01, 120, 120, 120, 120, 120];
  var options = {
    device: this.devices.sphero,
    command: 0x12,
    success: function() { console.log("Collision Detection Activated"); },
    error:  function(err) { console.log("Collision Detection Activation Error", err); },
    data: data
  };
  this.send(options);
  return this;
